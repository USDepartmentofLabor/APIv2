...
class Request extends CI_Controller {
	...
	public function index() {
		...
		$data["subtitle"] = "APIv2 - Admin Request";
	...
	private function data_process() {
		...
		if ($response == NO_DUPLICATE_ADMIN) {
			...
			//$this->email->cc($email);
			$this->email->subject("APIv2 AdminUI Request");
			$body = "
				...
				<tr>
				<td>Admin Request</td>
				<td>{$name} has requested access to APIv2 AdminUI Dashboard. <br/>Please " .anchor("".base_url()."login", "Login"). " to the dashboard to approve this request.</td>
				</tr>
				</table>
			";
			...
			// set default attributes
			$data["subtitle"] = "APIv2 AdminUI - Admin Request";
			$data["action"] = "request/access_process";
			$data["request_content"] = "request_view/signup_form";
			// set user message
